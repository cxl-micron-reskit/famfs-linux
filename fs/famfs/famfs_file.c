 ...
 * @iomap:  To be filled in to indicate where to find the right memory, relative
 *          to a dax device.
 * @offset: Within the file where the fault occurred (will be page boundary)
...
famfs_meta_to_dax_offset(struct inode *inode, struct iomap *iomap,
			 loff_t offset, off_t len, unsigned int flags)
{
	struct famfs_file_meta *meta = inode->i_private;
	int i;
	...
	for (i = 0; i < meta->tfs_extent_ct; i++) {
		...
		if (local_offset < dax_ext_len) {
			 ...
			 * Outputs:
			 * iomap->addr:   the offset within the dax device where the
			 *                data starts
