...
famfs_get_tree(struct fs_context *fc)
{
	...
	if (sb->s_root) {
		pr_notice("%s: found a match for %s\n", __func__, fc->source);
	...
	if (!dax_devp) {
		pr_err("%s: device %s not found or not dax\n",
		       __func__, fc->source);
		err = -ENODEV;
		goto deactivate_out;
	}
	/* XXX: is there a "put" or other cleanup needed from dax_dev_get()? */

	pr_notice("%s: dax_devp=%llx\n", __func__, (u64)dax_devp);
	...
	BUG_ON(fc->root);
	fc->root = dget(sb->s_root); /* XXX do I need a dput() somewhere? */
	return err;

deactivate_out:
	pr_err("%s: deactivating sb=%llx\n", __func__, (u64)sb);
